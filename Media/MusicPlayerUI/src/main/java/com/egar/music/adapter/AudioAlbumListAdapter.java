...
public class AudioAlbumListAdapter<T> extends BaseArrAdapter<T> implements SectionIndexer {
    // TAG
    ...
    public AudioAlbumListAdapter(@NonNull Context context, int resource) {
    ...
    private void setPlayingMedia(ProAudio playingMedia) {
        try {
            ...
            this.mPlayingAlbum = mPlayingMedia.getAlbum();
        } catch (Exception e) {
        }
    }
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = mLayoutInflater.inflate(R.layout.activity_music_list_frag_albums_item, parent, false);
        ...
        if (tItem instanceof ProAudio) {
            ...
            holder.tvIdx.setText(String.valueOf((position + 1)));
            //(3)
            FilterMedia filterMedia = (FilterMedia) tItem;
            holder.tvDesc.setText(filterMedia.sortStr);
            //(4)
            holder.ivEnd.setVisibility(View.INVISIBLE);

            try {
                if (TextUtils.equals(mPlayingAlbum, filterMedia.sortStr)) {
                    holder.tvIdx.setTextColor(mHlFontColor);
                    holder.tvDesc.setTextColor(mHlFontColor);
                    holder.ivStart.setImageResource(R.drawable.icon_album_c);
                } else {
                    holder.tvIdx.setTextColor(mNormalFontColor);
                    holder.tvDesc.setTextColor(mNormalFontColor);
                    holder.ivStart.setImageResource(R.drawable.icon_album);
                }
            } catch (Exception e) {
            }
        }
